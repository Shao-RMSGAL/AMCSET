...
void EMAGIC(float X, float ATOMD, float& ELOSS) {
    // X IS THE CURVE FITTING FROM TRIM AND UNIT IS EV, X UNIT IS KEV
    X *= 1000;
    if( X <= 1 ) {
        throw std::runtime_error("Invalid value of X provided to EMAGIC()");
    } else if(X > 1.0 && X <= 10.0){
        ELOSS = ATOMD*(.7122+.1026*X)/1000.0;
    } else if(X <= 100.0) {
        ELOSS = ATOMD*(1.671+.0252*X)/1000.0;
    } else if(X <= 400.0) {
        ELOSS = ATOMD*(5.0767+.0044 *X)/1000.0;
    } else if(X<=10000.0) {
        ELOSS = ATOMD*(9.28+.00144*X)/1000.0;
    } else {
        ELOSS = ATOMD*(9.28+.00144*X)/1000.0;
    }
}

float F(float X,float COLUMBIAVK,float Z1,float Z2,float AU) {
    //===========================UNIVERAIAL SCREENING POTENTIAL=================
    if(X == 0){
        return 0;
    } else {
        return COLUMBIAVK*X*(.35*std::exp(-.3/X/AU)+.55*std::exp(-1.2/X/AU)+.1*std::exp(-6.0/X/AU));
    }
    //=========================COLUMBIA POTENTIAL==============================
}

float DF(float X,float COLUMBIAVK,float Z1,float Z2,float AU) {
    return F(X,COLUMBIAVK,Z1,Z2,AU)/X+COLUMBIAVK/X*(.35*std::exp(-.3/X/AU)*.3/AU+.55*std::exp(-1.2/X/AU)*1.2/AU+.1*std::exp(-6.0/X/AU)*6.0/AU);
}


void TMAGIC(float MASS1, float Z1, float MASS2, float Z2, float INELAB, float P, float& ATOMD, float& THETA1RELATIVE, float& THETA2RELATIVE, float& RE) {

    float PI = 3.1415926;
    //=============================INITIAL PARAMETER============================

    float COLUMBIAVK = .0143992*Z1*Z2; //POTENTIAL V=COLUMBIAVK(ANSTRON*KEV)*Z1*Z2/R //
    float MC = MASS1*MASS2/(MASS1+MASS2); //MC--REDUCED MASS IN CENTER-MASS COORDINATOR
    float INVLAB = std::sqrt(INELAB*2/MASS1); //INVLAB--INCIDENT VOLOCITY IN LAB C.
    float EC = std::pow(1.0/2.0*MC*INVLAB,2); //EC--INITIAL ENERGY IN CM
    float AU = std::pow(.8854*.529/(std::sqrt(Z1)+std::sqrt(Z2)),2.0/3.0);
    float ELINHARD = EC*AU/COLUMBIAVK;

    //==============FIND RMIN FOR DIFFERENT ENERGY======================
    float AA = std::pow(P, 2);
    if( AA == 0 ) AA = .00001;

    float BB = COLUMBIAVK/EC;
    float CC = -1.0;
    float COLUMRMIN = 1.0/2.0/AA*(-BB+std::sqrt(std::pow(BB,2)-4*AA*CC)); //COLUMRMIN IS 1/RMIN, RMIN IS THE MINIMUM R UNDER COLUMBIA POTENTIAL

    int CALTIME = 1.0;
    three_hundred:
        float RMINTRY1 = COLUMRMIN;
        float DV = std::abs(-DF(RMINTRY1,COLUMBIAVK,Z1,Z2,AU)/EC-2*std::pow(P,2)*RMINTRY1);
        if(std::abs(DV) < .000001) DV = .1;
        int RMINTRY2 = RMINTRY1+(1-F(RMINTRY1,COLUMBIAVK,Z1,Z2,AU)/EC-std::pow(P,2)*std::pow(RMINTRY1,2))/DV;
        COLUMRMIN = RMINTRY2;
        CALTIME = CALTIME + 1;
        if(CALTIME > 10000) goto three_hundred;
        if(std::abs(RMINTRY2 - RMINTRY1) > .00001) goto three_hundred;
    three_hundred_fifty:
        int RMIN = (RMINTRY2+RMINTRY1)/2.0;
    ...
    float RBIERSACK = 2*(EC-F(RMIN,COLUMBIAVK,Z1,Z2,AU))/std::pow(RMIN,2)/DF(RMIN,COLUMBIAVK,Z1,Z2,AU);

    float BBIERSACK = P/AU;
    float ROBIERSACK = 1/(RMIN*AU);
    float RCBIERSACK = RBIERSACK/AU;

    float C1BIERSACK = .6743;
    float C2BIERSACK = .009611;
    float C3BIERSACK = .005175;
    float C4BIERSACK = 10;
    float C5BIERSACK = 6.314;
    float ALTHABIERSACK = 1+std::pow(C1BIERSACK*ELINHARD,(-1.0/2.0));
    float BELTABIERSACK = (C2BIERSACK+std::sqrt(ELINHARD))/(C3BIERSACK+std::sqrt(ELINHARD));
    float GAMABIERSACK = (C4BIERSACK+ELINHARD)/(C5BIERSACK+ELINHARD);
    float ABIERSACK = 2*ALTHABIERSACK*ELINHARD*std::pow(BBIERSACK,BELTABIERSACK);
    float GBIERSACK = GAMABIERSACK*1/ (std::sqrt(1.0 + std::pow(ABIERSACK, 2))-ABIERSACK);
    float DELTABIERSACK = ABIERSACK*(ROBIERSACK-BBIERSACK)/(1+GBIERSACK);
    float CALPHA1;
    if(P == 0) {
        CALPHA1 = PI;
    } else {
        CALPHA1 = 2*std::atan(std::sqrt(std::pow((ROBIERSACK+RCBIERSACK),2)/std::pow((BBIERSACK+RCBIERSACK+DELTABIERSACK),2)-1));
    }
    // ===================DEFLECTION ANGLE IN LAB COORDINATOR===========
    float COSPLUSMASS = std::cos(CALPHA1)+MASS1/MASS2;
    if(COSPLUSMASS = 0) THETA1RELATIVE=PI/2.0;
    if(COSPLUSMASS < 0) THETA1RELATIVE=PI+std::atan(std::sin(CALPHA1)/COSPLUSMASS);
    if(COSPLUSMASS > 0) THETA1RELATIVE = std::atan(std::sin(CALPHA1)/COSPLUSMASS);

    //===================CALCUTE T & DIRECTION OF TARGET ATOM=============
    RE = 4*EC*MC/MASS2*std::sin(CALPHA1/2.0)*std::sin(CALPHA1/2.0);
    //RE IS ENERGY TRANSIMITED TO TARGET ATOM//
    //===================RECOILED DIRECTION==================================
    THETA2RELATIVE = (PI-CALPHA1)/2.0; //RECOILED DIRECTION
}
...
int main() {
    ...
    float INELAB = 1.0;
    float P = 1.0;
    float THETA1RELATIVE = 1.0;
    float THETA2RELATIVE = 2.0;
    float RE = 1.0;
    TMAGIC(MASS1,Z1,MASS2,Z2,INELAB,P,ATOMD,THETA1RELATIVE,THETA2RELATIVE,RE);
    std::cout << "Values: " << ATOMD << " " << THETA1RELATIVE << " " << THETA2RELATIVE << " " << RE << std::endl;
    return 0;
}
...
FOR SIMUL = 1 TO SIMULS    ‘starting the bombardment 
    ...
    IF Ptype(rum) = 0 THEN ‘check if the new collision is for an atom, instead of an electron
        ...
        ZP(rum + 1) = ZP(rum) + L *std::cos(THETAP(rum))   ‘assign new depth to target
        XP(rum + 1) = XP(rum) + L *std::sin(THETAP(rum)) *std::cos(ALPHAP(rum)) ‘assign new X to target 
        YP(rum + 1) = YP(rum) + L *std::sin(THETAP(rum)) *std::sin(ALPHAP(rum)) ‘assign new Y to target
    ...
    IF Ptype(rum) = 1 THEN   ‘”1” means the particle is electron
        ...
        FOR ANGLE1 = 1 TO (Divisor - 1) ‘dividing 180 degrees by Divisor, performing integration
            ...
            IF ANGLE1 = 1 THEN TotalCross = TotalCross + DiffCross * 2 * PI *std::sin(PI / (1 - 10) * (1 - 10 ^ (1 / Divisor))) * ((PI / 2 / (1 - 10) * (1 - 10 ^ (2 / Divisor))) + (PI / 2 / (1 - 10) * (1 - 10 ^ (1 / Divisor))))  ‘integration concerning the first interval which starts from angle=0
            IF ANGLE1 > 1 AND ANGLE1 < (Divisor - 1) THEN TotalCross = TotalCross + DiffCross * 2 * PI *std::sin(PI / (1 - 10) * (1 - 10 ^ (ANGLE1 / Divisor))) * ((PI / 2 / (1 - 10) * (1 - 10 ^ ((ANGLE1 + 1) / Divisor))) - (PI / 2 / (1 - 10) * (1 - 10 ^ ((ANGLE1 - 1) / Divisor))))  ‘integration for angle intervals excluding two boundary points
            IF ANGLE1 = (Divisor - 1) THEN TotalCross = TotalCross + DiffCross * 2 * PI *std::sin(PI / (1 - 10) * (1 - 10 ^ ((Divisor - 1) / Divisor))) * (PI - (PI / 2 / (1 - 10) * (1 - 10 ^ ((Divisor - 1) / Divisor))) - (PI / 2 / (1 - 10) * (1 - 10 ^ ((Divisor - 2) / Divisor))))  ‘integration concerning the last angle point concerning the boundary at 180 degree
            //    P1 = DiffCross
            //   P2 = TotalCross
        ...
        FOR ANGLE2 = 1 TO (Divisor - 1)   ‘go through each angle point from 0 to 180
            ...
            IF ANGLE2 = 1 THEN ForThetaSelect1 = ForThetaSelect0 + DiffCross / TotalCross * 2 * PI *std::sin(PI / (1 - 10) * (1 - 10 ^ (1 / Divisor))) * ((PI / 2 / (1 - 10) * (1 - 10 ^ (2 / Divisor))) + (PI / 2 / (1 - 10) * (1 - 10 ^ (1 / Divisor))))    ‘Integration concerning first point at angle=0 needs to be specially treated. 
            IF ANGLE2 > 1 AND ANGLE2 < (Divisor - 1) THEN ForThetaSelect1 = ForThetaSelect0 + DiffCross / TotalCross * 2 * PI *std::sin(PI / (1 - 10) * (1 - 10 ^ (ANGLE2 / Divisor))) * ((PI / 2 / (1 - 10) * (1 - 10 ^ ((ANGLE2 + 1) / Divisor))) - (PI / 2 / (1 - 10) * (1 - 10 ^ ((ANGLE2 - 1) / Divisor)))) ‘integration for the middle points without two angle boundaries
            IF ANGLE2 = (Divisor - 1) THEN ForThetaSelect1 = ForThetaSelect0 + DiffCross / TotalCross * 2 * PI *std::sin(PI / (1 - 10) * (1 - 10 ^ ((Divisor - 1) / Divisor))) * (PI - (PI / 2 / (1 - 10) * (1 - 10 ^ ((Divisor - 1) / Divisor))) - (PI / 2 / (1 - 10) * (1 - 10 ^ ((Divisor - 2) / Divisor))))  ‘integration concerning the last boundary needs to be specially treated
...
‘lines 615 to 622 is to go through the flying distance group, from the last one to the first one, randomly pick one before the current one and switch the value. This is used to disorder the ordered scattering angle and randomly assign it them to different free flying distances. 
        ...
        FOR trial6 = 1 TO flyC  ‘go through each free flying distance in the group
            ...
            ElecREup = ((EP(rum) + 511) * (SIN(RA(trial6))) ^ 2 + MASSSUB * 931 * 1000 * (1 -std::cos(RA(trial6)))) * EP(rum) * (EP(rum) + 2 * 511)   ‘for calculation of energy transfer 
        ...
        FOR trial9 = 1 TO flyC  ‘for each collision after each flying distance, calculate the direction 
            ZP(rum + trial9) = ZP(rum + trial9 - 1) + RN(trial9) * 10 ^ 8 *std::cos(THETAP(rum + trial9 - 1))
            XP(rum + trial9) = XP(rum + trial9 - 1) + RN(trial9) * 10 ^ 8 *std::sin(THETAP(rum + trial9 - 1)) *std::cos(ALPHAP(rum + trial9 - 1))
            YP(rum + trial9) = YP(rum + trial9 - 1) + RN(trial9) * 10 ^ 8 *std::sin(THETAP(rum + trial9 - 1)) *std::sin(ALPHAP(rum + trial9 - 1))
...
SUB AMAGIC (THETAO, ALPHAO, THETA1RELATIVE, THETA2RELATIVE)
    ...
    //=====FOR PRIMARY ION
    X1 =std::sin(THETA1RELATIVE) *std::cos(ALPHA1RELATIVE)
    Y1 =std::sin(THETA1RELATIVE) *std::sin(ALPHA1RELATIVE)
    Z1 =std::cos(THETA1RELATIVE)
    ...
    IF Z > O THEN THETA1 =std::atan((std::sqrt(X ^ 2 + Y ^ 2) / Z)
    IF Z = 0 THEN THETA1 = PI / 2
    IF Z < 0 THEN THETA1 = PI +std::atan((std::sqrt(X ^ 2 + Y ^ 2) / Z)

    IFstd::sin(THETA1) <> 0 THEN
        IF X > 0 THEN ALPHA1 =std::atan((Y / X)
        IF X = 0 THEN ALPHA1 = PI - sign(Y) * PI / 2
        IF X < 0 THEN ALPHA1 = PI +std::atan((Y / X)
    ELSE
        ALPHA1 = 0
    END IF
    ...
    IF Z > O THEN THETA2 =std::atan((std::sqrt(X ^ 2 + Y ^ 2) / Z)
    IF Z = 0 THEN THETA2 = PI / 2
    IF Z < 0 THEN THETA2 = PI +std::atan((std::sqrt(X ^ 2 + Y ^ 2) / Z)

    IFstd::sin(THETA2) <> 0 THEN
        IF X > 0 THEN ALPHA2 =std::atan((Y / X)
        IF X = 0 THEN ALPHA2 = PI - sign(Y) * PI / 2
        IF X < 0 THEN ALPHA2 = PI +std::atan((Y / X)
    ELSE
        ALPHA2 = 0
    END IF
...
SUB DSCMOTT (mott(), scr(), ElectronEnergy, SubstrateZ, Theta)
    ...
    FOR xx = 1 TO 5
        ...
        NEXT yy
        Rmott = Rmott + alpha1 * (1 -std::cos(Theta)) ^ ((xx - 1) / 2)
    NEXT xx
    DSC = (SubstrateZ * re1) ^ 2 * (1 - beta ^ 2) / beta ^ 4 * (1 -std::cos(Theta)) ^ (-2) //Differential Ruth cross section
    moment = 2 * beta * gamma * elecmass * Vc *std::sin(Theta / 2)
    Feq = 0
    FOR ww = 1 TO 3
