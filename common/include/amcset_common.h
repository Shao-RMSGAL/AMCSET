...
class Ion : public Particle {
   public:
     ...
     */
    length_quantity interatomic_spacing(
...
class Layer final {
   public:
     ...
     */
    size_t get_number_of_components() const noexcept {
   ...
   public:
    ...
    struct Settings {
        ...
        const bool enable_damage_cascade_;  //!< Control whether damange cascade
                                            //!< is enabled
        const energy_quantity
            ion_stopping_energy_;  //!< Energy at which ions stop
        const energy_quantity
        ...
        const size_t
            bombardment_count_;  //!< Number of bombardments to simulate

        const bool
            is_electron_;  //!< Determines if bombardment is for an electron
