   ...
   protected:
    ...
    Simulation create_simulation() const try {
        ...
        auto ion_displacement_energy =
            energy_quantity(0.04 * kilo_electron_volt);
...
TEST_F(CommonTest, isotope_data_get_isotope_mass_test) try {
    ...
    ASSERT_EQ(IsotopeData::get_isotope_mass(0, 0),
              double(1.0) * constants::m_e);
...
TEST_F(CommonTest, layer_test) try {
    const auto water_layer =
        create_water_layer(length_quantity(1.0 * angstrom));
    ...
    ASSERT_THROW(
        Layer(std::move(empty_vec), double(-1.0) * depth, mass_density),
        std::invalid_argument);
    ...
    ASSERT_EQ(
        water_layer.get_number_density(),
        mass_density / (((2.0 / 3.0) * IsotopeData::get_isotope_mass(1, 1) +
...
TEST_F(CommonTest, simulation_and_settings_test) try {
    ...
    ASSERT_EQ(simulation.get_settings(
                  &Simulation::Settings::electron_stopping_energy_),
    ...
    ASSERT_EQ(
        ...
        ion_stopping_energy);
    ASSERT_EQ(simulation.get_settings(
                  &Simulation::Settings::ion_displacement_energy_),
              ion_displacement_energy);
    ASSERT_EQ(simulation.get_settings(
                  &Simulation::Settings::log_single_displacement_),
    ...
    ASSERT_EQ(
        simulation.get_settings(&Simulation::Settings::bombardment_count_),
        bombardment_count);
...
TEST_F(CommonTest, electronic_stopping_energy_test) try {
    ...
    auto charge = charge_quantity(14.0 * elementary_charge);
    ...
    auto final_energy =
        test.electronic_stopping_energy(charge, mass, energy, layer);
